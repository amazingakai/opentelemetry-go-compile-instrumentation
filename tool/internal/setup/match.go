...
func (sp *SetupPhase) loadRules(ctx context.Context, moduleDirs map[string]bool) ([]rule.InstRule, error) {
	...
	// Load rules from config files (.otel.yml) if available
	configFiles := findConfigFiles(moduleDirs)
	if len(configFiles) > 0 {
		sp.Debug("rules source: config files", "configFiles", configFiles)
		return nil, ex.New("config files found but not supported yet")
	}
